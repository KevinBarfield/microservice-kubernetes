...
docker buildx build --target=CATALOG --push --platform linux/amd64,linux/arm64 --tag=$DOCKER_ACCOUNT/microservice-kubernetes-demo-catalog .
...
docker buildx build --target=CUSTOMER --push --platform linux/amd64,linux/arm64 --tag=$DOCKER_ACCOUNT/microservice-kubernetes-demo-customer .
...
docker buildx build --target=ORDER --push --platform linux/amd64,linux/arm64 --tag=$DOCKER_ACCOUNT/microservice-kubernetes-demo-order .
